...
main <- function(N, P, Q=1, R=1, frq=.05, lnk=NL, eps=.1, oks=~LN1, ...)
{
    ...
    evl <- with(dat$evl,
    {
        ...
        ret <- CL(ret, OPM=vpd(rsp, knl, dvp$OPM$par))
...
test <- function()
{
    u <- main(N=500, P=10000, Q=2, R=1, frq=.1, eps=.5, vcf=c(1, 1), oks=~LN2)
