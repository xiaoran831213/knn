...
main <- function(N, P, Q=1, R=1, frq=.05, lnk=I1, eps=.1, oks=~LN1, ...)
{
    ...
    dvp <- with(dat$dvp,
    {
        ...
        ret <- CL(ret, JL2=PMQ(rsp, krn(gmx, ~JL2)))
        ret <- CL(ret, JP2=NMQ(rsp, krn(gmx, ~JP2)))
        ret <- CL(ret, JP3=NMQ(rsp, krn(gmx, ~JP3)))
        ret <- CL(ret, PC2=NMQ(rsp, krn(gmx, ~PC2)))
        ret <- CL(ret, PC3=NMQ(rsp, krn(gmx, ~PC3)))
    ...
    evl <- with(dat$evl,
    {
        ...
        ret <- CL(ret, JL2=vpd(rsp, krn(gmx, ~JL2), dvp$JL2$par))
        ret <- CL(ret, JP2=vpd(rsp, krn(gmx, ~JP2), dvp$JP2$par))
        ret <- CL(ret, JP3=vpd(rsp, krn(gmx, ~JP3), dvp$JP3$par))
        ret <- CL(ret, PC2=vpd(rsp, krn(gmx, ~PC2), dvp$PC2$par))
        ret <- CL(ret, PC3=vpd(rsp, krn(gmx, ~PC3), dvp$PC3$par))
