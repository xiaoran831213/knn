...
source('sim/lnk.R')
test.lnk <- function(hist=0, seed=100)
{
    set.seed(seed)
    ...
    g <- g + ylab(expression(symbol(eta) %~% symbol(f)))
    g <- g + xlab(expression(tilde(symbol(eta)) %~% symbol(g)))
    if(hist == 1)
        g <- g + geom_histogram(aes(x=x, y=6*..density..), d0, fill='gray', alpha=.5, binwidth=.5)

    g <- g + geom_point(aes(x=x, y=x), d0, color='gray', alpha=.01)
    g <- g + geom_point(aes(x=y, y=x, color=d), d0, alpha=.01)

    if(hist == 1)
        g <- g + geom_histogram(aes(x=y, y=6*..density.., fill=d), d0, color='black', binwidth=.5)
