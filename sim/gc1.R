...
gct.rml <- function(y, K, X=NULL, maxit=100)
{
    ## temporary directory
    ...
    ## intercept prepend to covariants
    cvr <- cbind(X.0=1, X)
    cvr.path <- file.pach(tpd, 'cvr.txt')
    write.table(cvr, cvr.path, quote=FALSE, sep='\t', row.names=FALSE, col.names=FALSE)
